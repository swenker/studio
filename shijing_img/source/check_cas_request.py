...
def count_validate_requests(dt,site):
    cmd = "grep ssoauth/samlValidate?TARGET= /home/jetty/current/logs/2015_11_%s.request.log |grep POST |grep %s |wc -l"  %(dt,site)
    print cmd
    for host in hosts:
        print host+"-->>",
        exec_ssh_cmd(get_ssh_client(host,22),cmd)
...
def count_verify_user_requests(dt,site):
    cmd = "grep /ssoauth/web/steps_to_verify.html /home/jetty/current/logs/2015_11_%s.request.log |grep svc=verify_user |grep %s |wc -l"  %(dt,site)
    print cmd
    for host in hosts:
        print host+"-->>",
        exec_ssh_cmd(get_ssh_client(host,22),cmd)
...
def count_signup_requests(dt,site):
    cmd = "grep /ssoauth/app/user/signup?type= /home/jetty/current/logs/2015_11_%s.request.log | grep POST |wc -l"  %(dt)
    print cmd
    for host in hosts:
        print host+"-->>",
        exec_ssh_cmd(get_ssh_client(host,22),cmd)
...
def count_login_requests(dt,site):
    cmd = "grep /ssoauth/login?TARGET= /home/jetty/current/logs/2015_11_%s.request.log | grep POST |grep %s |wc -l"  %(dt,site)
    print cmd
    for host in hosts:
        print host+"-->>",
        exec_ssh_cmd(get_ssh_client(host,22),cmd)
