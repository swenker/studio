...
warning_valve=0
critical_value=0
while getopts "H:p:s:i:o:w:c:" Input;
do
       case ${Input} in
       H)      host=${OPTARG};;
       p)      port=${OPTARG};;
       ...
       i)      interface=${OPTARG};;
       o)      output=${OPTARG};;
       w)      warning_valve=${OPTARG};;
       c)      critical_valve=${OPTARG};;
...
if [ $(($return_bytes_out)) -gt $(($critical_valve)) ];then
        STATUS_TXT="Critical"
        STATUS_CODE=2
elif [ $(($return_bytes_out)) -ge $(($warning_valve)) ]; then
        STATUS_TXT="Warning"
        STATUS_CODE=1
fi
#echo $(($critical_valve)),$(($warning_valve)) 
